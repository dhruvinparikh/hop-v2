...
/// @notice Shared helper functions for Tempo-based tests in hop-v2
/// @dev Ported from frax-oft-upgradeable/test/foundry/helpers/TempoTestHelpers.sol
abstract contract TempoTestHelpers is Test {
    ...
    /// @dev Create a TIP20 token with a DEX pair for swap tests
    ...
    ///      Places bid (PATH_USD for token) AND ask (token for PATH_USD)
    ///      so both swap directions work in tests.
