...
/// @title HopV201Tempo
/// @notice Tempo-flavored sibling of `HopV201`. Behaviorally identical to `HopV201`
///         (RECOVER_ROLE-gated `recoverERC20`, no unbounded `recover()`), with two
///         deliberate divergences for Tempo:
///
///         1. `recoverETH` is intentionally omitted. Tempo settles all fees in TIP20 /
///            ERC20 via EndpointV2Alt, so the contract has no native ETH surface to recover.
///
...
///         so a proxy that previously delegated to a HopV2-derived implementation can be
///         upgraded to a HopV201Tempo-derived implementation in place.
/// @author Frax Finance: https://github.com/FraxFinance
contract HopV201Tempo is AccessControlEnumerableUpgradeable, IHopV201 {
    ...
    /// @dev keccak256("RECOVER_ROLE")
    bytes32 public constant RECOVER_ROLE = 0x62b337eaefec74dadf1a62e856bf9db4f14a0f27d4f48156a95a9f98e7d5e066;
    ...
    function _setRemoteHop(uint32 _eid, bytes32 _remoteHop) internal {
        _getHopV2Storage().remoteHop[_eid] = _remoteHop;
    }

    function setNumDVNs(uint32 _numDVNs) public onlyRole(DEFAULT_ADMIN_ROLE) {
        _getHopV2Storage().numDVNs = _numDVNs;
    }

    function setHopFee(uint256 _hopFee) public onlyRole(DEFAULT_ADMIN_ROLE) {
        _getHopV2Storage().hopFee = _hopFee;
    }

    function setExecutorOptions(uint32 eid, bytes memory _options) public onlyRole(DEFAULT_ADMIN_ROLE) {
        _getHopV2Storage().executorOptions[eid] = _options;
    }

    /// @notice Recover an ERC20 / TIP20 token stuck on this contract.
    /// @dev Mirrors `HopV201.recoverERC20`. Tempo has no native ETH surface so `recoverETH`
    ///      is intentionally omitted — there is nothing to recover on a chain whose fees
    ///      are settled in TIP20 via EndpointV2Alt.
